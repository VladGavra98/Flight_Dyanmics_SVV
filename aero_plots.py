...
# CLtheory=np.linspace(0,1.2,500)
# a1,a2,a3=np.polyfit(CL,CD,2)
# CDtest=[]

for i in range(0,len(CDtheory)):
    a=m.sqrt((CDtheory[i]-CD0)/grad)
    #b=a1*CLtheory[i]**2+a2*CLtheory[i]+a3
    #CDtest.append(b)
    CL2theory.append(a)
e=1/(grad*m.pi*AR)
...
# ax.text(-0.7, 0.93, '$R_e$: '+str('{:.3E}'.format(Decimal(min(Re))))+' - '+str('{:.3E}'.format(Decimal(max(Re))))+'\n'
#                     'M: '+str(round(min(M),3))+' - ' +str(round(max(M),3))+'\n'
#                     'Clean configuration',style='italic',bbox={'facecolor': 'white', 'edgecolor':'black'})
...
# ax.text(0.022, 0.84, '$R_e$: '+str('{:.3E}'.format(Decimal(min(Re))))+' - '+str('{:.3E}'.format(Decimal(max(Re))))+'\n'
#                     'M: '+str(round(min(M),3))+' - ' +str(round(max(M),3))+'\n'
#                     'Clean configuration',style='italic',bbox={'facecolor': 'white', 'edgecolor':'black'})
# plt.savefig('Plots\Dragpolar.png')
#
# plt.scatter(CD,CL)
# plt.plot(CDtest,CLtheory)
print(alpha)
print(CD)
plt.show()
